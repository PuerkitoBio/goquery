...
//   example, given
//   `struct { T map[string]S `goquery:"#someId,[foo],[bar],[baz]"``, `[foo]`
//   will be used to determine the string map key,but `[bar]` and `[baz]` will
//   be ignored, with the tags present on the S struct type taking precedence.
//
